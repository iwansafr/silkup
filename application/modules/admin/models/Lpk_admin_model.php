...
class Lpk_admin_model extends CI_Model
{
	public function my_lpk()
	{
		if(!empty($_SESSION))
		{
			...
			if(empty($lpk))
			{
				$lpk = $this->db->query('SELECT JSON_EXTRACT(param,"$.lpk_id") AS id FROM user_member WHERE user_id = ?',$_SESSION[base_url().'_logged_in']['id'])->row_array();
			}
